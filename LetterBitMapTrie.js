...
class LetterBitMapTrie {
    ...
    for (const word of this.dictionary) {
      for (let i = 0; i < word.length; ++i) {
        for (let j = i + 1; j < word.length; ++j) {
          const slice = word.slice(i + 1, j + 1);
  ...
  prepend() {
    const prepend = Object.create(null);
  ...
  append() {
    const append = Object.create(null);
  ...
  remove(view, key, value) {
    this.map[key][view] &= ~(1 << (value.charCodeAt(0) - "a".charCodeAt(0)));
    this.map[key][2] = 0;
  }
  ...
  computePerfectPlay(letters) {
    ...
    const prepends = this.get(0, letters);
    for (const p of prepends) {
      if (this.perfectPlay(p + letters) === 1) {
        return -1;
      }
    }
    const appends = this.get(1, letters);
  ...
  recomputeWinner(letters) {
    const previous = this.map[letters][2];
    this.map[letters][2] = 0;
    const updated = this.computePerfectPlay(letters);
  ...
  winningStarts() {
    ...
    const wins = letters.filter((letter) => this.perfectPlay(letter) == 1);
